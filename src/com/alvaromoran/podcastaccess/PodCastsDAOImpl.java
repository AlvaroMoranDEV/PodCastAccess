...
import com.alvaromoran.podcastaccess.constants.GenericITunesConstants;
import com.alvaromoran.podcastaccess.constants.ITunesSearchKeys;
import com.alvaromoran.podcastaccess.constants.ITunesSpecificPodCastKeys;
import com.alvaromoran.podcastaccess.data.dto.PodCastChannelDTO;
import com.alvaromoran.podcastaccess.data.json.JsonRoot;
...
import com.alvaromoran.podcastaccess.factories.PodCastsFactory;
import com.alvaromoran.podcastaccess.factories.PodCastsFactoryImpl;
...
public class PodCastsDAOImpl implements PodCastsDAO {
    ...
    private final PodCastsFactory podCastsFactory;
    ...
    public PodCastsDAOImpl() {
        ...
        this.podCastsFactory = new PodCastsFactoryImpl();
...
//region PodCastsDAO implementation
    ...
    public List<PodCastChannelDTO> updateArtistSearchParameter(String artist) throws PodCastAccessUriException {
        // Store value
        if (artist == null || "".equalsIgnoreCase(artist)) {
    ...
    public PodCastChannelDTO getEnrichedChannelInformation(PodCastChannelDTO selectedChannel, boolean getEpisodes) throws PodCastAccessConnectionException {
        if (selectedChannel != null) {
            ...
            if (channelAdditionalInfo != null && !getEpisodes) {
                this.podCastsFactory.createChannelExtended(selectedChannel, channelAdditionalInfo);
            } else if (channelAdditionalInfo != null) {
                this.podCastsFactory.createChannelExtendedAndEpisodes(selectedChannel, channelAdditionalInfo);
            }
        }
    ...
    public PodCastChannelDTO getListOfEpisodesFromChannel(PodCastChannelDTO selectedChannel) {
        if (selectedChannel != null) {
            Document channelAdditionalInfo = executeUriForFeed(selectedChannel.getFeedUrl());
            selectedChannel = this.podCastsFactory.createChannelWithEpisodes(selectedChannel, channelAdditionalInfo);
        }
        return selectedChannel;
    }
    ...
    private List<PodCastChannelDTO> executeUriForITunes() throws PodCastAccessConnectionException, PodCastAccessUriException {
        ...
        if (url != null && isValidUri(url)) {
            ...
            roughChannels.forEach((roughChannel) -> {
                PodCastChannelDTO parsedChannel = this.podCastsFactory.createChannel(roughChannel, this.searchPaidChannels);
